...
function solveMUMPS!(A::SparseMatrixCSC, rhs::AbstractArray, x::Array,
	                 sym::Int=0,ooc::Int=0,tr::Int=0)
...
function factorMUMPS(A::SparseMatrixCSC{ComplexF64},sym::Int=0,ooc::Int=0)
    # Generate LU-factorization of complex matrix A, 'sym' can be:
	# 0=unsymmetric, 1=symm. pos def, 2=general symmetric
...
function factorMUMPS(A::SparseMatrixCSC{Float64},sym::Int=0,ooc::Int=0)
    # Generate LU-factorization of a real matrix A, 'sym' can be:
	# 0=unsymmetric, 1=symm. pos def, 2=general symmetric
    if size(A,1) != size(A,2)
		error("factorMUMPS: Matrix must be square!")
    end
...
function applyMUMPS(factor::MUMPSfactorization{T1},rhs::AbstractArray{T2,N},
	     x::Array=zeros(promote_type(T1,T2),size(rhs)),tr::Int=0) where {T1,T2,N}
...
ccall( (:solve_mumps_cmplx_sparse_rhs_, MUMPSlibPath),
			...
			Ref(ptr), Ref(nzrhs), Ref(nrhs), convert(Ptr{ComplexF64},
			pointer(rhs.nzval)), rhs.rowval, rhs.colptr, convert(Ptr{ComplexF64}, pointer(x)), Ref(tr))
return x
end
